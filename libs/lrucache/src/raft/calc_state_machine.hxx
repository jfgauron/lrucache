...
public:
    calc_state_machine(lrucache::cache_config config, bool async_snapshot = false)
        : cur_value_(0)
    ...
    static void dec_log(buffer& log, op_payload& payload_out) {
        std::cout << "dec_log is called!" << std::endl;
...
private:
    ...
    void create_snapshot_internal(ptr<snapshot> ss) {
        ...
        if (last_snapshot_) {
            auto prev_index = last_snapshot_->get_last_log_idx();
            std::string filename = "snapshot" + std::to_string(prev_index) + ".bin";
            std::remove(filename.c_str());
        }

        last_snapshot_ = ss;
        // TODO: Write state_machine snapshot to file
    ...
    void serialize_state(lrucache::cache_state state) {

    }
    
    // State machine's current value.
    std::atomic<int64_t> cur_value_;
    lrucache::cache_state state_;

    // Last committed Raft log number.
    std::atomic<uint64_t> last_committed_idx_;

    // Snapshot context information
    // ptr<snapshot_ctx> snapshot_ctx_;
